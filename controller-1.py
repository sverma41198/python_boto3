...
def check_and_create_security_group(region,group_name, group_description, vpc_id):
    ec2 = boto3.client('ec2', region_name= region)  # Replace 'your_region' with your desired AWS region
...
def launch_ec2_instance(region,ami_id,vpc_id, subnet_id, security_group_id):
    ec2 = boto3.client('ec2', region_name= region)  # Replace 'your_region' with your desired AWS region
    # Instance details
...
def main(region, ami_id):
    ...
    created_vpc_id, created_subnet_id= create_vpc_igw_route_table_public_subnet(
        region,desired_vpc_name, desired_vpc_cidr, desired_subnet_cidr
    )
    ...
    existing_or_created_group_id = check_and_create_security_group(
        region,desired_group_name, desired_group_description, desired_vpc_id
    )
...
if __name__ == '__main__':
    ...
    if region in [region[0] for region in All] :
        ami_id= All[[region[0] for region in All].index(region)][1]
        main(region,ami_id)
